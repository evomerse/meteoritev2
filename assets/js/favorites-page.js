  ...
  if (!favorites || favorites.length === 0) {
    container.innerHTML = `
      <div class="empty-state">
        ...
        <div class="empty-state-text">Ajoutez des villes à vos favoris pour consulter rapidement leur météo</div>
      </div>
    `;
    return;
  }
    ...
    try {
      ...
      card.innerHTML = `
        <div class="favorite-card-header">
          ...
          <button class="favorite-remove" onclick="window.removeFavoriteConfirm('${favorite.id}')" title="Retirer des favoris">
            ✕
          </button>
        </div>
      ...
      console.error(`Error loading weather for ${favorite.city_name}:`, error);
    }
  }
}
...
window.removeFavoriteConfirm = async function(favoriteId) {
  ...
  if (confirm('Êtes-vous sûr de vouloir retirer cette ville de vos favoris ?')) {
    try {
